...
export default function AdminPage() {
  ...
  async function handleImageUpload(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    recipeFormRef.current = { ...recipeFormRef.current, image: result };
    setMessage('Image selected. Click Save changes to store it.');
    event.target.value = '';
  }
        ...
        <section className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
              ...
              {filteredRecipes.length ? filteredRecipes.slice(0, 12).map((recipe) => (
                <div key={recipe.slug} className="rounded-3xl border border-stone-200 bg-stone-50 p-5 shadow-sm">
                  ...
                  <div className="mt-4 flex flex-wrap items-center gap-2">
                    ...
                    <button type="button" onClick={() => loadRecipeForEdit(recipe)} className="rounded-full border border-stone-300 bg-white px-3 py-1 text-xs font-semibold text-stone-700 hover:bg-stone-100">Edit</button>
          ...
          <aside className="rounded-3xl border border-stone-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-8 rounded-3xl border border-stone-200 bg-stone-50 p-5">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  <h3 className="text-lg font-semibold text-stone-900">Bulk recipe import</h3>
                  <p className="mt-1 text-sm text-stone-600">Download the sample Excel sheet, fill in the required recipe fields, and upload it here to add many recipes at once.</p>
                </div>
              ...
              <div className="mt-4 rounded-2xl border border-dashed border-stone-300 bg-white p-4">
                <label className="block text-sm text-stone-700">
                  <span className="mb-2 block font-medium text-stone-900">Upload filled Excel file</span>
                  <input type="file" accept=".xlsx,.xls" onChange={handleBulkImport} className="w-full rounded-2xl border border-stone-300 bg-stone-50 p-3" />
                </label>
            ...
            <div className="mt-8 rounded-3xl border border-stone-200 bg-stone-50 p-5">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <h3 className="text-lg font-semibold text-stone-900">AI settings</h3>
              ...
              <div className="mt-5 space-y-4">
                <label className="flex items-center gap-3 rounded-2xl border border-stone-300 bg-white p-3">
