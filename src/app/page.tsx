...
import { recipes, cuisines } from '@/lib/recipes';
  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(180deg,#fffaf6_0%,#fffefb_45%,#fff7ed_100%)] text-stone-900 transition-colors duration-200 dark:bg-[linear-gradient(180deg,#111827_0%,#1f2937_45%,#111827_100%)] dark:text-stone-100">
      <section className="mx-auto grid max-w-7xl gap-8 px-6 py-10 lg:grid-cols-[1.1fr_0.9fr] lg:px-10">
        <article className="glass-card rounded-3xl p-8 soft-ring dark:border-stone-800 dark:bg-stone-900/95">
          <p className="text-sm uppercase tracking-[0.35em] text-amber-700">Discover the world, one recipe at a time</p>
          <h1 className="mt-4 text-4xl font-semibold text-stone-900 dark:text-stone-100 lg:text-6xl">Global Recipe Hub</h1>
          <p className="mt-4 max-w-2xl text-lg text-stone-600 dark:text-stone-300">Explore recipes from Pakistan, Italy, Thailand, Turkey, India, and beyond with a premium, fully alcohol-free approach and AI-assisted food science guidance.</p>
        ...
        <aside className="rounded-3xl border border-stone-800 bg-stone-950 p-8 text-stone-100 shadow-[0_24px_60px_rgba(15,23,42,0.35)]">
          ...
          <img src={featured.image} alt={featured.title} className="mt-5 h-48 w-full rounded-3xl object-cover" />
          <Link
            href={`/recipe-of-the-day?recipe=${featured.slug}`}
            className="mt-5 inline-block rounded-full bg-amber-500 px-4 py-2 text-stone-950"
          >
            View full recipe
          </Link>
        </aside>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-8 lg:px-10">
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
          {recipes.slice(0, 4).map((recipe) => (
            <Link key={recipe.slug} href={`/recipes/${recipe.slug}`} className="glass-card rounded-3xl p-4 transition duration-200 hover:-translate-y-1 hover:shadow-xl dark:border-stone-800 dark:bg-stone-900/95">
              <img src={recipe.image} alt={recipe.title} className="h-36 w-full rounded-2xl object-cover" />
